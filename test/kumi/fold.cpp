...
TTS_CASE("Check tuple::fold_left behavior")
{
  ...
  auto accumulated  = kumi::fold_left ( [](auto acc, auto m)
                                        {
...
TTS_CASE("Check tuple::fold_left constexpr behavior")
{
  ...
  constexpr auto accumulated  = kumi::fold_left(  []<std::size_t N>( std::array<int,N> acc, auto m)
                                                  {
                                                    if constexpr(N==0)
                                                    {
...
TTS_CASE("Check tuple::fold_right behavior")
{
  ...
  auto accumulated  = kumi::fold_right( [](auto acc, auto m)
                                        {
                                          acc.push_back(sizeof(m));
                                          return acc;
                                        }
...
TTS_CASE("Check tuple::fold_right constexpr behavior")
{
  ...
  constexpr auto accumulated  = kumi::fold_right( []<std::size_t N>( std::array<int,N> acc, auto m)
                                                  {
                                                    if constexpr(N==0)
                                                    {
                                                      std::array<int,1> res{sizeof(m)};

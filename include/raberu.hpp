...
namespace rbr
{
  ...
  namespace detail
  {
    ...
    template<typename Key, typename C> constexpr decltype(auto) link(linked_value<C> &v) noexcept
    {
      return v;
    }

    template<typename Key, typename C>
    constexpr decltype(auto) link(linked_value<C> const &v) noexcept
    {
      return v;
    }
  ...
  template<typename... Ts> struct settings
  {
    ...
    constexpr settings(Vs &&...v) : content_(detail::link<tag_t<std::decay_t<Vs>>>(RBR_FWD(v))...)
    {
    }
